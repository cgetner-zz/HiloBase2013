...
class ChangeWorkEnvQues < ActiveRecord::Migration
  def self.up
	execute "TRUNCATE table workenv_questions"
    
    work_env = WorkenvQuestion.new(:order=>1,:question=>"TO FIND APPROPRIATE VENUES, YOU...",:xscoring=>"NEG",:yscoring=>"NEG",:description_left=>"Get together with friends <br /> and discuss previously <br />visited locations.",:description_right=>"Determine venue options based upon budget and estimated guest list.",:for_emp=>0)
    work_env.save
    ...
    work_env.save
    work_env = WorkenvQuestion.new(:order=>3,:question=>"TO PLAN THE SURPRISE, YOU...",:xscoring=>"POS",:yscoring=>"POS",:description_left=>"Create a detailed schedule <br />of events.",:description_right=>"Call people to find out how they would like to participate.",:for_emp=>0)
    work_env.save
    ...
    work_env.save
    work_env = WorkenvQuestion.new(:order=>5,:question=>"TO GET SNACKS FOR 100 PEOPLE, YOU...",:xscoring=>"POS",:yscoring=>"NEG",:description_left=>"Hire a caterer and ask for <br />their recommendations.",:description_right=>"Take time to craft a menu specifically for the party.",:for_emp=>0)
    work_env.save
    work_env = WorkenvQuestion.new(:order=>6,:question=>"THE VENUE LACKS DECORATIONS, YOU...",:xscoring=>"POS",:yscoring=>"no",:description_left=>"Create a budget and hire a professional to decorate <br />the venue.",:description_right=>"Create decorations and <br />set-up the venue together <br />with friends.",:for_emp=>0)
    work_env.save
    work_env = WorkenvQuestion.new(:order=>7,:question=>"TO PICK MUSIC FOR THE PARTY, YOU...",:xscoring=>"NEG",:yscoring=>"POS",:description_left=>"Make a mix or playlist of favorite songs.",:description_right=>"Find and hire a DJ or do <br />it yourself.",:for_emp=>0)
    work_env.save
    work_env = WorkenvQuestion.new(:order=>8,:question=>"SO THAT THINGS GO ACCORDING TO PLAN, YOU...",:xscoring=>"POS",:yscoring=>"POS",:description_left=>"Distribute the schedule <br />of events.",:description_right=>"Call individuals to discuss <br />their roles.",:for_emp=>0)
    work_env.save
    work_env = WorkenvQuestion.new(:order=>9,:question=>"WE STILL NEED A CAKE! YOU...",:xscoring=>"POS",:yscoring=>"NEG",:description_left=>"Find a bakery nearby and get <br />a cake.",:description_right=>"Review top bakeries in town.",:for_emp=>0)
    work_env.save
